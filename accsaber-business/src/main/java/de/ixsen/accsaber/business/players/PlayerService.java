package de.ixsen.accsaber.business.players;

import de.ixsen.accsaber.business.APUtils;
...
public class PlayerService implements HasLogger {
    ...
    public void loadPlayerScores() {
        ...
        this.getLogger().info("Loading scores for {} players.", allPlayers.size());
        List<BeatMap> rankedMaps = this.mapRepository.findAllRankedMaps();
    ...
    @Transactional
    protected void handlePlayer(List<BeatMap> rankedMaps, Player player) {
    ...
    private void handleSetScores(List<BeatMap> rankedMaps, Player player, List<Score> newlySetScores, ScoreSaberScoreDto scoreSaberScoreDto) {
    ...
    // fixme if necessary
    private void handleRankedMaps(Score score, List<BeatMap> rankedMaps, ScoreSaberScoreDto scoreSaberScoreDto) {
        Optional<BeatMap> potentialRankedMap = rankedMaps.stream().filter(r -> Objects.equals(r.getLeaderboardId(), score.getLeaderboardId())).findFirst();
        ...
        if (potentialRankedMap.isPresent()) {
            ...
            double ap = APUtils.calculateApByAcc(score.getAccuracy(), potentialRankedMap.get().getComplexity());
            score.setAp(ap);
        }
    }
